...
> For the deep technical write-up of the HGT pipeline (graph schema,
> training loop, ablation logic), see [`NEW/README.md`](NEW/README.md).
...
|---|---|---|
| **HGT pipeline** | `NEW/src/` | Trains the marriage-prediction model on ablated graph; emits cohort JSONs that the dashboard reads. |
| **Backend (FastAPI)** | `NEW/viz-mas/server/` | Serves cohort data, hosts the 6-round bilateral LLM-agent negotiation, accepts/restores match commits. |
| **Frontend (Vue 3 + Vite)** | `NEW/viz-mas/src/` | Six linked views (V1–V6) for cohort overview, processed pairs, embedding space, bipartite detail, agent arena, and rule injector. |
...
|---|---|---|
| `NEW/data/raw/DS0001/27063-0001-Data.rda` | ICPSR study #27063 | manual download (Step 2) |
| `NEW/data/raw/DS0003/27063-0003-Data.rda` | ICPSR study #27063 | manual download (Step 2) |
| `NEW/data/raw/DS0003/event_value_labels.json` | already in git (small JSON) | bundled |
| `NEW/data/raw/DS0009/*.rda`, `DS0011/*.rda` | ICPSR study #27063 | manual download (optional — macro signal) |
| `NEW/data/processed/hgt_pipeline/*.parquet` | generated | `python -m src.main --stage data` |
| `NEW/data/processed/ds0003/*.parquet` | generated | first call to `events_loader` |
| `NEW/checkpoints/best.pt` | generated | `python -m src.main --stage train` |
| `NEW/runs/<timestamp>/metrics.json` | generated | `python -m src.main --stage eval` |
| `NEW/viz/data/cohort_<year>__<ablation>.json` | generated | `python -m src.main --stage eval` |
...
The codebase lives under `NEW/`; the legacy top-level `src/` is a copy
maintained for backward compatibility. Treat `NEW/` as the source of truth.
...
4. Place them under `NEW/data/raw/`:
...
Create `NEW/.env` (or set in your shell) for the optional Qwen key:
...
├── src/                                    legacy mirror of NEW/src/
├── tests/                                  pipeline smoke tests
├── scripts/                                legacy diagnostics
└── viz/                                    legacy static viewer (unused by viz-mas)
```
...
- **Frontend layout & view contracts** — V1–V6 design notes:
  [`NEW/viz-mas/README.md`](NEW/viz-mas/README.md)
- **CMGPD-LN itself** — ICPSR codebooks under `NEW/data/raw/DS000X/27063-000X-Codebook.md`
